...
class SecurityQuestionShcema(str, Enum):
    ...
    @classmethod
    def get_desciption(cls, value: "SecurityQuestionShcema") -> str:
        description = {
...
class RoleChoiceShcema(str, Enum):
    CUSTOMER = "customer"
...
class BaseUserSchema(SQLModel):
    ...
    security_question: SecurityQuestionShcema = Field(max_length=30)
    security_answer: str = Field(max_length=30)
    account_status: AccountStatusSchema = Field(default=AccountStatusSchema.INACTIVE)
    role: RoleChoiceShcema = Field(default=RoleChoiceShcema.CUSTOMER)


class UserCreateScheam(BaseUserSchema):
        ...
        if "password" in values.data and v != values.data["password"]:
            raise HTTPException(
                ...
                detail={
                    ...
                    "action": "pls ensure that the password you entered match",
                },
            )
        return v
