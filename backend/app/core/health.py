...
class HealthCheck:
    ...
    async def add_service(
        ...
        # FIX: validate_dependencies must run BEFORE acquiring the lock
        # to avoid deadlock from awaiting inside a held lock
    ...
    async def check_celery(self) -> bool:  # FIX: indented back inside the class
        try:
            ...
            # FIX: check broker first, then workers — both wrapped in to_thread
            broker_up = await asyncio.to_thread(check_broker)
            if not broker_up:
                return False
    ...
    async def check_service_health(self, service_name: str) -> ServiceStatus:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                async with self._lock:
                    ...
                    self._cached_status = None  # FIX: invalidate cache on degraded
        ...
        async with self._lock:
            ...
            self._cached_status = None  # FIX: invalidate cache on unhealthy
            self._last_check_time = None
            logger.error(
    ...
    async def clean_up(self) -> None:  # FIX: renamed from cleanup to match main.py
        async with self._lock:
