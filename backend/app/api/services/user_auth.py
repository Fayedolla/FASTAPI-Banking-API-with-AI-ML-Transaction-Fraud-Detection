...
class UserAuthService:
    ...
    async def verify_user_password(self, password: str, hashed_password: str) -> bool:
    ...
    async def reset_user_state(
        ...
        *,
        ...
        if log_action and previous_status != user.account_status:
            logger.info(
                "User {user.email} state reset: {previous_status} -> {user.account_status}"
            )
        ...
        try:
            ...
            for attempt in range(3):
                try:
                    ...
                    logger.info(f"OTP sent to this {user.email} successfully")
                    return True, otp
                except Exception as e:
                    logger.error(
                        f"Failed to send OTP email (attempt {attempt + 1}): {str(e)}"
                    )
            ...
            logger.error(f"Failed to generate and save OTP: {str(e)}")
            user.otp = ""
            user.otp_expiry_time = None
            await session.commit()
            await session.refresh(user)
    ...
    async def create_user(
        ...
        try:
            ...
            logger.error(
                f"Failed to sent activatoin email to {new_user.email}: {str(e)}"
            )
            raise
    ...
    async def activate_user_account(
        ...
        try:
            ...
            if user.is_active:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail={"status": "error", "message": "User is already activated"},
                )
    ...
    async def increment_failed_login_attempts(
        ...
        if user.failed_login_attempts >= settings.LOGING_ATTEMPTS:
            ...
            try:
                ...
                logger.error(
                    f"Failed to send account lockout email to {user.email}: {str(e)}"
                )
            logger.warning(
    ...
    async def reset_password(
        ...
        try:
            ...
            await session.commit()
            await session.refresh(user)
            logger.info(f"Password reset successful for the user: {user.email}")
